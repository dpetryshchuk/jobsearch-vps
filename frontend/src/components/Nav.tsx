...
import { MessageSquare, Users, BarChart2, FileText, Activity, Database } from 'lucide-react'
...
export function BottomNav() {
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-20 bg-background border-t border-border">
      <div className="flex">
        {mainNavItems.map(({ to, label, icon: Icon }) => (
          <NavLink
            key={to}
            ...
            className={({ isActive }) =>
              cn(
                'flex-1 flex flex-col items-center gap-1 py-3 text-[10px] font-medium tracking-wide transition-colors',
